...
var _Sources = (() => {
  ...
  var NHentai = class _NHentai {
    ...
    // Initialize filterEnabled and hiddenIds with default values
    this.stateManager.store("filterEnabled", true).catch((err) => {
        console.error("Failed to initialize filterEnabled:", err);
    });
    this.stateManager.store("hiddenIds", []).catch((err) => {
        console.error("Failed to initialize hiddenIds:", err);
    });
}    // Sourrce Settings
    async getSourceMenu() {
    const isFilterEnabled = (await this.stateManager.retrieve("filterEnabled")) ?? true;

    // Initialize `filterEnabled` if not present
    if (await this.stateManager.retrieve("filterEnabled") === undefined) {
        await this.stateManager.store("filterEnabled", true);
    }
    ...
    return Promise.resolve(App.createDUISection({
        ...
        rows: () => Promise.resolve([
            App.createDUISwitch({
                id: "filter_toggle",
                label: "Filter Hidden Manga",
                value: App.createDUIBinding({
                    get: () => isFilterEnabled,
                    set: async (newValue) => await this.stateManager.store("filterEnabled", newValue),
                }),
            }),
    ...
    // Safely retrieve hiddenIds
    const hiddenIds = new Set((await this.stateManager.retrieve("hiddenIds")) || []);
    hiddenIds.add(mangaId);

    try {
        await this.stateManager.store("hiddenIds", Array.from(hiddenIds));
    } catch (err) {
        console.error("Failed to store hiddenIds:", err);
    }
    ...
    let jsonData;
    try {
        jsonData = this.parseJson(response);
    } catch (err) {
        console.error("Failed to parse JSON in getChapterDetails:", err);
        throw new Error("Chapter details could not be retrieved.");
    }
    ...
    const isFilterEnabled = (await this.stateManager.retrieve("filterEnabled")) ?? true;
    const hiddenIds = (await this.stateManager.retrieve("hiddenIds")) || [];
    ...
    const promises = sections.map(async (section) => {
        ...
        try {
            ...
            console.error("Failed to parse JSON in getHomePageSections:", err);
            return;
        }

        if (!hasNoResults(jsonData)) {
            let items = parseSearch(jsonData);

            if (isFilterEnabled) {
                items = items.filter((item) => !hiddenIds.includes(item.mangaId));
            }
